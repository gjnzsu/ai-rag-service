...
### 4.4 可选的查询 embedding 缓存

Vector/Hybrid 查询和 Graph 语义起点查询在调用 embedding API 前，可先检查进程内缓存。实际输入文本、模型配置及隔离范围完全一致，且条目未过期、未被淘汰时，复用之前生成的向量；不同说法不会因为语义相似而命中。

缓存采用 Python `OrderedDict` + `Lock` 实现 LRU + TTL，没有引入 Redis；默认关闭，开启后默认容量为 1,024 条、TTL 为 900 秒。它跨请求存活，但不同 worker 不共享，重启清空。普通查询和 Graph 使用各自范围，不跨两条路径共享条目。

命中后仍执行本次索引／快照检索及后续回答生成。Graph 查询向量可以跨快照复用，但不能让旧快照自动变新。架构图在两条检索路径标出 `Q` 接入点，下方的 `Q QUERY EMBEDDING CACHE` 细节区展开命中、未命中和关闭缓存三条路径；索引构建的 embedding 不经过该缓存。

缓存键、实现细节、启用方式及测量边界见 [查询 embedding 缓存设计与实验](query-embedding-cache.md)。
