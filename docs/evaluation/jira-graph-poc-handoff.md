...
状态：实现与用户验收完成，正式收尾。更新日期：2026-09-10。详见 [用户验收记录](jira-graph-user-acceptance.md)。
...
- 回答模型：`gpt-5.5-2026-04-23`，真实生成与引用样例已通过。Embedding 不变；OpenAI reranker 默认模型也已统一为 GPT-5.5，provider 默认仍为 none。
...
用户已确认六类场景的整体测试完成。下表记录整体验收确认，不代替逐题量化评分；直接可见的截图案例见用户验收记录。

| # | 问题 | 验收重点 | 用户结果 |
| --- | --- | --- | --- |
| 1 | AIPLAT 有哪些 Epic？每个 Epic 下有多少 Story 和 Bug？ | 全景与 Jira 一致；统计不受 top-k 影响 | 用户整体验收确认 |
| 2 | AIPLAT-17 下有哪些事项？哪些完成、哪些未完成？ | 下钻、状态、来源可追溯 | 用户整体验收确认 |
| 3 | AIPLAT-37 阻塞了谁？双方目前什么状态？ | 37 BLOCKS 23；双方 To Do；引用真实关系 | 用户整体验收确认 |
| 4 | AIPLAT-46 与 AIPLAT-16 有什么依赖？是否仍未完成？ | 46 BLOCKS 16；双方 Done；不误报为当前未完成阻塞 | 用户整体验收确认 |
| 5 | 与“检索返回未经授权的知识内容”有关的 backlog 是哪项？有哪些依赖事项？ | 不输入 key 定位 46，并扩展到 16；核对检索证据 | 用户整体验收确认 |
| 6 | 根据 backlog，为什么 AIPLAT-37 阻塞 AIPLAT-23？哪些明确记录、哪些无法确认？ | 区分正文事实、关系记录与推测；不足时克制回答 | 用户整体验收确认 |

每题记录：实际答案、来源引用、事实是否正确、证据边界是否清楚、问题备注。支持的结构查询与预设意图需要在页面选择相应入口；本版不包含任意自然语言意图规划。

## 后续独立实验（2026-09-21）

后续 Agentic Epic 分析已实现并通过本地用户验收，见 [使用指南](../guides/agentic-epic-analysis.md) 和 [对照实验结果](agentic-epic-analysis-results.md)。它使用独立可选入口；下文与前述验收记录仍描述原 Graph POC 的历史范围，不代表 Agentic 已随旧提交进入 main 或部署。

## 已知限制与后续候选

- 依赖检索尚未附带 Epic 归属证据，同时问依赖与双方 Epic 的复合问题可能拒答；可在全景观察跨 Epic 连接。自动补齐归属是后续候选，不是本次收尾新增任务。
- 2026-09-09 的 GPT-5.5 探针报告记录三次真实问题调用，其中两次 supported，一次因缺少 Epic 归属证据返回 insufficient_evidence；用户已完成本轮业务验收；完整问题集生成对照、独立量化人工评分、负载测试及实际账单统计仍未完成。
...
实现、架构图与模型更新已提交并推送至 main（截至 0d1b636）。为保留本地演示数据和运行环境，worktree 暂时保留。部署不在本轮操作范围。
